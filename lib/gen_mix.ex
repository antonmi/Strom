defmodule Strom.GenMix do
  ...
  def handle_cast({:new_data, {name, fun}, chunk}, %__MODULE__{} = mix) do
    Enum.each(mix.consumers, fn {{name, fun}, cons} ->
  ...
  def handle_cast({:consumer_got_data, {name, fun}}, %__MODULE__{} = mix) do
    Enum.each(mix.producers, fn {{name, fun}, task} ->
